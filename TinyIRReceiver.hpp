...
#else
void handleReceivedIRData(uint16_t aAddress, uint8_t aCommand,
		bool isRepetition);
#endif
...
void IRPinChangeInterruptHandler(void)
	...
	uint16_t tMicrosOfMarkOrSpace = tCurrentMicros
			- TinyIRReceiverControl.LastChangeMicros;
	...
	if (tIRLevel == LOW) {
		...
		if (tState == IR_RECEIVER_STATE_WAITING_FOR_START_MARK) {
			...
			if (tMicrosOfMarkOrSpace >= lowerValue25Percent(NEC_HEADER_SPACE)
					&& tMicrosOfMarkOrSpace
							<= upperValue25Percent(NEC_HEADER_SPACE)) {
				/*
			...
			} else if (tMicrosOfMarkOrSpace
					>= lowerValue25Percent(NEC_REPEAT_HEADER_SPACE)
					&& tMicrosOfMarkOrSpace
							<= upperValue25Percent(NEC_REPEAT_HEADER_SPACE)
			...
			if (tMicrosOfMarkOrSpace >= lowerValue(NEC_ZERO_SPACE)
				...
				if (tMicrosOfMarkOrSpace >= 2 * NEC_UNIT) {
					...
					TinyIRReceiverControl.IRRawData.ULong |=
							TinyIRReceiverControl.IRRawDataMask;
				} else {
		...
		if (tState == IR_RECEIVER_STATE_WAITING_FOR_START_SPACE) {
			...
			if (tMicrosOfMarkOrSpace >= lowerValue25Percent(NEC_HEADER_MARK)
					&& tMicrosOfMarkOrSpace
							<= upperValue25Percent(NEC_HEADER_MARK)) {
			...
			if (tMicrosOfMarkOrSpace >= lowerValue(NEC_BIT_MARK)
				 ...
				 */
				if (TinyIRReceiverControl.IRRawDataBitCounter >= NEC_BITS
						|| TinyIRReceiverControl.IRRepeatDetected) {
					/*
					...
					handleReceivedTinyIRData(
							...
							TinyIRReceiverControl.IRRawData.UByte.MidHighByte,
							TinyIRReceiverControl.IRRepeatDetected);
...
bool isTinyReceiverIdle() {
	return (TinyIRReceiverControl.IRReceiverState
